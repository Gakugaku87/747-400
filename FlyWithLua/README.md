...
3. S/Cまで0.5 NM、またはARM済みの地点が通過して`NOW`になるとMCP高度を設定します。
4. 0.35秒後、747の実際のALTセレクターコマンドを発行します。
5. 747本体側が新しい巡航高度を受理したことを巡航高度datarefで確認します。
...
- VNAV Descentではない
- AutopilotサーボがON
- 高度保持が成立している（`altitude_hold_status == 2`）
- 現在高度が巡航高度の±500 ft以内（747本体のALTセレクター受理条件と同じ）
...
- 受理を確認した同一Step高度は再実行しません。コマンドを発行しただけでは実行済みにしません。
